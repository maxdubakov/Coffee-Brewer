...
struct StageCreationChoice: View {
    // MARK: - Properties
    let onManualChoice: () -> Void
    let onRecordChoice: () -> Void
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Choice cards
            VStack(spacing: 20) {
                // Record option - highlighted as primary
                Button(action: onRecordChoice) {
                    VStack(spacing: 20) {
                        ZStack {
                            Circle()
                                .fill(LinearGradient(
...
#Preview {
    NavigationStack {
        StageCreationChoice(
            onManualChoice: { print("Manual selected") },
            onRecordChoice: { print("Record selected") }
        )
    }
}
