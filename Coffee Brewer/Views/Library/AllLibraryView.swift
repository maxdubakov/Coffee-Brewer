...
struct AllLibraryView: View {
    ...
    @State private var selectedCoffees: Set<NSManagedObjectID> = []
    ...
    private var filteredCoffees: [Coffee] {
        let allCoffees = Array(coffees)
        if searchText.isEmpty {
            return allCoffees
        } else {
            return allCoffees.filter { coffee in
                let nameMatch = coffee.name?.localizedCaseInsensitiveContains(searchText) ?? false
                let roasterMatch = coffee.roaster?.name?.localizedCaseInsensitiveContains(searchText) ?? false
    ...
    private var filteredBrews: [Brew] {
        ...
        if searchText.isEmpty {
            ...
            return allBrews.filter { brew in
                ...
                return nameMatch || roasterNameMatch || notesMatch
            }
        }
    }

    private var hasAnyItems: Bool {
        !filteredCoffees.isEmpty || !filteredRoasters.isEmpty || !filteredGrinders.isEmpty || !filteredBrews.isEmpty
    }

    private var totalSelectedItems: Int {
        selectedCoffees.count + selectedRoasters.count + selectedGrinders.count + selectedBrews.count
    }
    ...
    var body: some View {
        ...
        .alert("Delete Roaster?", isPresented: $navigationCoordinator.showingDeleteRoasterAlert) {
            ...
            Text("This will also delete all associated coffees. This action cannot be undone.")
        }
        .alert("Delete Grinder?", isPresented: $navigationCoordinator.showingDeleteGrinderAlert) {
            ...
            Text("Associated brews will keep their data but lose grinder reference.")
        }
    ...
    private var emptyStateView: some View {
        CenteredContent(verticalOffset: -70) {
            VStack(spacing: 16) {
                ...
                if searchText.isEmpty {
                    Text("Add coffees, roasters, grinders, and track your brews")
                        .font(.system(size: 14))
    ...
    private func toggleCoffeeSelection(for coffee: Coffee) {
        if selectedCoffees.contains(coffee.objectID) {
            selectedCoffees.remove(coffee.objectID)
        } else {
            selectedCoffees.insert(coffee.objectID)
        }
    }
    ...
    private func deleteSelectedItems() {
        withAnimation {
            // Delete coffees
            for objectID in selectedCoffees {
                if let coffee = viewContext.object(with: objectID) as? Coffee {
                    viewContext.delete(coffee)
                }
            }
